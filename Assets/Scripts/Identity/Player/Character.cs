...
public class Character : Identity
{
    ...
    {
        // ตรวจสอบและกำหนดค่า mapGenerator หากยังไม่ได้กำหนด
        ...
        if (mapGenerator == null)
        {
            Debug.LogError("MapGenerator is not assigned to Character!");
        }
    }
    protected void GetRemainEnergy()
        ...
        public virtual void Move(Vector2 direction)
        {
            ...
            if (HasPlacement(toX, toY))
            {
                ...
                else if (IsBoss(toX, toY))
                {
                    Debug.Log("Boss");
                    mapGenerator.bosses[toX, toY].Hit();
                }
                else if (IsSmallCurrency(toX, toY))
                {
                    Debug.Log("SmallCurrency");
                    mapGenerator.smallCurrencies[toX, toY].Hit();
                }
                else if (IsBigCurrency(toX, toY))
                {
        ...
        public bool IsBoss(int x, int y)
        {
            int mapData = mapGenerator.GetMapData(x, y);
            return mapData == mapGenerator.boss;
        }
        ...
        public bool IsSmallCurrency(int x, int y)
        {
            int mapData = mapGenerator.GetMapData(x, y);
            return mapData == mapGenerator.smallCurrency;
        }

        public bool IsBigCurrency(int x, int y)
        {
            int mapData = mapGenerator.GetMapData(x, y);
